...
uint32_t getLineOops(int owner) {
...
void updateRecordScCommands(int type, int endtype, uint32_t oldPointer, uint32_t newPointer) {
...
bool pcsx2upload() {
    ...
    for(int i = 0; i < Records.size(); i++) {
        ...
        for(int k = 0; k < 17; k++) {
            ...
            for(e_suggestline_t &line : variant.lines) {
                ...
                if(!PALMode) {
                    ...
                    for(int t = 0; t < 4; t++) {ps2line.localisations[t] = (SUBMode) ? line.localisations[t] : uint32_t(~0);}
                    ps2line.owner = line.owner;
                    ps2line.timestamp_start = line.timestamp_start;
                    ps2line.timestamp_end = line.timestamp_end;
                    ps2line.ptr_oops = getLineOops(line.owner);//line.ptr_oops;
                    ...
                    for(int t = 0; t < 7; t++) {ps2lineP.localisations[t] = (SUBMode) ? line.localisations[t] : uint32_t(~0);}
                    ps2lineP.owner = line.owner;
                    ps2lineP.timestamp_start = line.timestamp_start;
                    ps2lineP.timestamp_end = line.timestamp_end;
                    ps2lineP.ptr_oops = getLineOops(line.owner);//line.ptr_oops;
        ...
        if(record.type == 1 && !VSMode) { // Good, Bad & Awful
            updateRecordScCommands(1, 3, record.address, uploadPos);
        } else { // Other scenes
            updateRecordScCommands(record.type, record.type, record.address, uploadPos);
        }
...
bool olmupload(QString filename) {
    ...
    bool result = pcsx2reader::writeExtended(OLM_LINK_ADDRESS, buffer, fileSize);
